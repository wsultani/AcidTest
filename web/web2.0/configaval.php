...
<?
  ...
  if ( $_POST["IPAddress"] != "" && $_POST["SMBSlot"] != "" && $_POST["BVSlot"] != "") {
    ...
    $avals = $_POST["SMBSlot"];
    $bvs = $_POST["BVSlot"];
    $query = "SELECT * FROM $table WHERE IPAddress = \"$ip\" AND SMBSlot = \"$avals\" AND BVSlot = \"$bvs\"";
    mysql_query ($query);
    $result = mysql_affected_rows();
    if ( $result > 0 ) {
      $stat = fail;
      $msg .= "aval system " . $ip . " with avalslot " . $avals . " and bvslot " . $bvs . " is already Registered.<br>";
      $empty ++;
    }
  }
...
<html>
  <head>
    ...
    <title>Bivio Test Center - Config Avalanche Device</title>
  </head>
  <body>
    <form method="post">
    <table border=0 cellpadding=0 cellspacing=0 valign=top align=center width=100%>
    <tr align=left><th><h2>Configure Avalanche Device</h2></th></tr>
<?
    if ( $msg != "" ) {
...
?>
    ...
    <input type=hidden name="Class" value="aval">
...
?>
    ...
    <tr><th>
      ...
      <tr>
      <td width=50% align=center><input type="submit" value="Register Avalanche"></td>
